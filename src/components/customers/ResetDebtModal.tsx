...
export function ResetDebtModal({ 
  ...
  // Filtrar clientes con saldo $0 o negativo (que han pagado)
  const eligibleCustomers = customers.filter(customer => {
    const balance = customer.movements.reduce((sum, m) => sum + m.amount, 0)
    return balance <= 0 // $0 o negativo
  })
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-xl p-6 z-50 w-[600px] max-w-[90vw] max-h-[80vh] overflow-y-auto">
          ...
          {productList.length > 0 && (
            <div className="mb-4 p-3 bg-orange-50 rounded-lg">
              <p className="text-sm text-orange-800">
                Creando nueva deuda con <span className="font-semibold">{productList.length}</span> producto{productList.length !== 1 ? 's' : ''} 
          ...
          <div className="relative mb-4">
            <input
              type="text"
              placeholder="Buscar cliente pagado..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full py-2 px-4 pl-10 rounded-md border border-slate-200 bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-500"
              autoFocus
            />
            <SearchIcon className="absolute left-3 top-[10px] text-slate-400" />
          </div>

          {/* Lista de clientes elegibles */}
          <div className="space-y-2 max-h-96 overflow-y-auto">
            ...
            {!isLoading && !error && filteredCustomers.length === 0 && (
              <div className="text-center py-8 text-slate-500">
                {searchTerm ? 'No se encontraron clientes pagados con ese nombre' : 'No hay clientes que hayan pagado sus deudas'}
              </div>
            )}
            ...
            {!isLoading && !error && filteredCustomers.map(customer => {
              ...
              return (
                <button
                  ...
                  className="w-full text-left p-3 border border-slate-200 rounded-lg hover:bg-orange-50 transition-colors duration-200"
                >
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-green-100 rounded-full">
                        <RestartAltIcon className="text-green-600" />
                      </div>
                      <div>
                    ...
                    <div className="text-right">
                      <div className="font-bold text-green-600">
                        Pagado: ${formatPrice(Math.abs(balance))}
                      </div>
